package xyz.gorelov.recorder.record
...
import xyz.gorelov.recorder.MainActivity
import xyz.gorelov.recorder.R
import xyz.gorelov.recorder.database.RecordDatabase
import xyz.gorelov.recorder.database.RecordDatabaseDao
import xyz.gorelov.recorder.database.RecordingItem
import java.io.File
...
class RecordService: Service() {
    ...
    private fun setFileNameAndPath() {
        ...
        do {
            mFileName = (getString(R.string.default_file_name) + "_" + dateTime
                    + count + ".mp4")
    ...
    private fun stopRecording() {
        ...
        mRecorder?.release()
        Toast.makeText(this,
            getString(R.string.toast_recording_finish), Toast.LENGTH_SHORT).show()

        recordingItem.name = mFileName.toString()
        recordingItem.filePath = mFilePath.toString()
        recordingItem.length = mElapsedMillis.toString()
